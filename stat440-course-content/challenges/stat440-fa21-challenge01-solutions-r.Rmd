...
Answers will vary, but must be two differences. Graded for completeness.
...
```{r}
nuisance02 <- nuisance %>%
  mutate(`Date Reported` = lubridate::mdy(`Date Reported`), `Date Notice Mailed or Given` = lubridate::mdy(`Date Notice Mailed or Given`), `File Close Date` = lubridate::mdy(`File Close Date`))

slice_head(nuisance02, n=10)
```
