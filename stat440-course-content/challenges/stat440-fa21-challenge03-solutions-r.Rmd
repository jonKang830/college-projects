...
slice_tail(covid, n=5)
slice_tail(election, n=5)
```
...
combined <- inner_join(covid03, election03, by="state")
...
covid04 <- sqldf("SELECT date, fips, cases, deaths, UPPER(state) AS state 
               FROM covid
               WHERE date = '2020-11-02'")
election04 <- sqldf("SELECT * 
                FROM election
                WHERE year='2016'
                GROUP BY state
                HAVING candidatevotes = max(candidatevotes)")

sqldf("SELECT date, covid04.state, cases, deaths, party_simplified, year
      FROM covid04, election04
      WHERE covid04.state = election04.state
      ORDER BY cases DESC
      LIMIT 10")
```
...
```{r}
sqldf("SELECT date, covid05.state, cases, deaths, party_simplified, year
      FROM (SELECT date, fips, cases, deaths, UPPER(state) AS state 
               FROM covid
               WHERE date = '2020-11-02') AS covid05, (SELECT * 
                FROM election
                WHERE year='2016'
                GROUP BY state
                HAVING candidatevotes = max(candidatevotes)) AS election05
      WHERE covid05.state = election05.state
      ORDER BY cases DESC
      LIMIT 10")
```
