...
big_spender <- trips %>%
  ...
  select(-WEEK) %>%
  group_by(PANID) %>%
  summarise(sum_dollars=sum(dollars999)) %>%
  filter(sum_dollars>15000)
...
comb0 <- inner_join(trips0,delivery,"IRI_KEY")

inner_join(comb0,big_spender, "PANID") %>%
  distinct(PANID, .keep_all = TRUE) %>%
  select(PANID, Market_Name)
```
...
dat2deliv <- inner_join(dat2,delivery,"IRI_KEY") %>% 
  filter(OU=="GR") %>% 
  summarise(totdols = sum(CENTS999, na.rm = TRUE)/100) %>% 
  filter(totdols >= 100) %>% 
  add_count(PANID) %>% 
  filter(n>2)

dat1dat2deliv <- inner_join(dat1,dat2deliv,"PANID")

z <- dat1dat2deliv %>% 
  select(PANID, `Children Group Code`, totdols, IRI_KEY, n) %>% 
  arrange(PANID)

z2 <- inner_join(z,delivery,"IRI_KEY") %>% 
  select(PANID, `Children Group Code`, totdols, IRI_KEY, n, OU)

#head(z2,10)

length(unique(z2$PANID))
```
...
combo_trips_delivery <- inner_join(trips0,delivery,"IRI_KEY")

combo_all <- inner_join(ads0,combo_trips_delivery,"PANID")

x=combo_all %>% 
  filter(WEEK < 1140, `Type of Residential Possession`==1, OU=="GR") %>%
  group_by(PANID, IRI_KEY) %>%  
  mutate(totdols = round(sum(CENTS999, na.rm = TRUE)/100,2)) %>% 
  filter(totdols > 500) 

#head(x,10)

length(unique(x$PANID))
...
#add_count counts the number of times this panelist goes to this store
xx <- combo_all %>% 
  filter(`Family Size`>1) %>% 
  group_by(PANID, IRI_KEY) %>% 
  mutate(totdols = round(sum(CENTS999, na.rm = TRUE)/100,2)) %>% 
  filter(totdols >= 1000) %>% 
  select(PANID, `Family Size`, totdols, IRI_KEY) %>% 
  distinct(PANID, .keep_all = TRUE) %>% 
  group_by(PANID) %>% 
  add_count(PANID) %>% 
  filter(n>1) %>% 
...
length(unique(xx$PANID))
```

Since the wording of the question does not specifically require the delivery stores data, that solution would result in 1329 panelists.
...
combo_all <- inner_join(ads0,trips0,"PANID")
