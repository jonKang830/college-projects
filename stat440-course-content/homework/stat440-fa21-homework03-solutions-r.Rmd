...
**#4** (Accessing and importing data) **Import the following datasets using a programming language and their corresponding data URLs, and print the structure of each.** Read the data descriptions to help verify the import was successful.
...
ccso_jail_data <- read_csv("https://uofi.box.com/shared/static/lurfpdqk63au11yzra4a88sxizda0iz5.dat", col_types = cols(booking_date = col_date(format = "%m/%d/%Y"), 
        booking_time = col_time(format = "%H:%M:%S"), 
        released_date = col_date(format = "%m/%d/%Y"), 
        released_time = col_time(format = "%H:%M:%S")))
slice_head(ccso_jail_data, n=10)

chicago_food_data <- read_csv("https://uofi.box.com/shared/static/lwwzlv2sulip1p8qoqqghmw0pouso0wk.csv", col_types = cols(`Inspection Date` = col_date(format = "%m/%d/%Y")))
slice_head(chicago_food_data, n=10)
```
...
Answers will vary, but must show the structure. Dates and times may have been treated as date or datetime formats in **Problem 4**.

```{r}
str(ccso_jail_data)
str(chicago_food_data)
```
...
```{r}
inspections <- chicago_food_data %>% 
  filter(`Inspection Date` > "2011-09-01") %>%
  mutate(pass_or_notf=ifelse(Results=="Fail", 0, Results), pass_or_notp=ifelse(pass_or_notf=="Pass", 1, pass_or_notf), pass_or_notpc=ifelse(pass_or_notp=="Pass w/ Conditions", 0, pass_or_notp), pass_or_not = ifelse(pass_or_notpc > 1, NA, pass_or_notpc)) %>% 
  select(-c(Address, `Census Tracts`, City, `Community Areas`, `Historical Wards 2003-2015`, `Location`, `State`, `Wards`, `Zip Codes`, pass_or_notf, pass_or_notp, pass_or_notpc)) %>% 
  drop_na()
...
Among the businesses that failed their inspections, the third most common facility type is school.

```{r}
head(sort( table( select( filter( inspections, Results=="Fail"), `Facility Type`)), decreasing = TRUE ))
```
...
```{r}
inmates <- ccso_jail_data %>% 
  mutate(booking_date_time=as.POSIXct(glue::glue("{booking_date} {booking_time}"), format="%Y-%m-%d %H:%M:%S"), released_date_time=as.POSIXct(glue::glue("{released_date} {released_time}"),format="%Y-%m-%d %H:%M:%S")) %>%
  mutate(hoursx = difftime(released_date_time,booking_date_time, units="hours"), years=as.vector(hoursx)%/%(24*365), days=as.vector(hoursx)%/%(24), hours=(as.vector(hoursx)%%(24))%/%(1), minutes=((as.vector(hoursx)*60)%%(60))%/%(1), seconds=((as.vector(hoursx)*60*60)%%(60))%/%(1)) %>%
  mutate(time_spent_in_jail1 = years + days*24 + hours + minutes/60 + seconds/(60^2), time_spent_in_jail2 = difftime(released_date_time,booking_date_time, units="hours")) %>%
  rename(booked_number=booking_number, booked_date=booking_date, booked_time=booking_time, booked_reason=booking_reason, booked_date_time=booking_date_time) %>%
  select(-hoursx)
...
Yes, "time_spent_in_jail1" and "time_spent_in_jail2" are practically the same.

```{r}
head(inmates$time_spent_in_jail1)
head(as.vector(inmates$time_spent_in_jail2))
all.equal(inmates$time_spent_in_jail1,as.vector(inmates$time_spent_in_jail2))
```
...
The crimes associated with the person who spent the most time in jail ("time_spent_in_jail1") are: AGGRAVATED DOMESTIC BATTERY, CRIMINAL SEXUAL ASSAULT, OTHER TRAFFIC OFFENSES, FLEEING & ELUDING OR ATTEMPTING TO FLEE & ELUDE, PAROLE REVOCATION, VIOLATION OF ORDERS OF PROTECTION.

```{r}
unique(inmates[which(inmates$time_spent_in_jail1==max(inmates$time_spent_in_jail1, na.rm=TRUE)),"crime"])
```
