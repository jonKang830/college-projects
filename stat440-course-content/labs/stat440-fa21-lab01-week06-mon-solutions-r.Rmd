...
## Week06 Lab01 Mon
### Only the driver should submit this assignment into their individual student repo. Save the filename as lab01-week06-mon-netID.Rmd. Then render this file to .html when completed. 
...
```{r}
library(tidyverse)
speed_data <- read_csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/speed-dating-data.csv?token=AAABJG3HPIGBUWGKW3SKYITBL4IFW")
str(speed_data)
```
...
```{r}
speed_data$abs_int_corr <- abs(speed_data$int_corr)
fields <- unique(speed_data$field)
avg_abs_int_corr <- rep(0, length(fields))
for (i in 1:length(fields)){
  avg_abs_int_corr[i] <- speed_data %>%
    filter(field==fields[i]) %>%
    select(abs_int_corr) %>%
    colMeans(.,na.rm=TRUE)
}
tibble(fields,avg_abs_int_corr)
```
...
Students in the field of Sociology have the smallest average date score, while those in Arts Administration have the largest average date score.

```{r}
speed_data <- speed_data %>%
  mutate(date_score = attr+sinc+intel+fun+amb+shar+like+prob)
fields <- unique(speed_data$field)
avg_date_score <- rep(0, length(fields))
for (i in 1:length(fields)){
  avg_date_score[i] <- speed_data %>%
    filter(field==fields[i]) %>%
    select(date_score) %>%
    colMeans(.,na.rm=TRUE)
}
tibble(fields,avg_date_score) %>%
  slice_min(avg_date_score)
tibble(fields,avg_date_score) %>%
  slice_max(avg_date_score)
```
