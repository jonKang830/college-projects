...
```{r}
library(tidyverse)
nuissance_data <- read_csv("https://data.urbanaillinois.us/api/views/tsn9-95m3/rows.csv?accessType=DOWNLOAD&bom=true")
```
...
```{r}
market_data <- read_delim("https://data.urbanaillinois.us/api/views/6gtk-bwms/rows.tsv?accessType=DOWNLOAD&bom=true","\t")
str(market_data)
```
...
```{r}
library(jsonlite)
congress_data0 <- fromJSON("https://theunitedstates.io/congress-legislators/legislators-current.json")
names(congress_data0)
congress_data <- congress_data0$name
str(congress_data)
```
