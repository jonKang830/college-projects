...
```{r}
names_f_h <- baby_names_data[str_detect(baby_names_data$name,"^[F-F][q-z]|^[G-G][a-z]|^[H-H][a-j]"),]
names_f_h[sample(nrow(names_f_h),10),]
```
...
The letter "y" (case insensitive) appears 139892 times among all female names.

```{r}
female_y_names <- baby_names_data %>%
  filter(sex=="F")

sum(str_count(female_y_names$name,"y|Y"))
```
