...
Only 1 female dater rates the importance of their datee being the same race as 10 out of 10.

```{r}
library(tidyverse)
library(sqldf) #runs the sqldf library for SQL with data frames
speed_data <- read_csv("https://uofi.box.com/shared/static/w8vjq29rig8pdvussyg3997h1gwhkimq.csv")
```

```{r}
tab03<-sqldf("SELECT iid 
      FROM speed_data
      WHERE gender = 0 AND imprace = 10
      GROUP BY iid
      ")
sqldf("SELECT COUNT(iid) 
      FROM tab03")
```
...
```{r}
date_score_success <- sqldf("
  SELECT *, attr+sinc+intel+fun+amb+shar+like+prob AS date_score
  FROM speed_data
  WHERE match==1
...
sqldf("
  SELECT date_score, samerace, age, age_o, goal
  FROM date_score_success
  LIMIT 10
      ")
```
...
There are 24 interracial matches occurring in the data.

```{r}
tab05 <- sqldf("SELECT iid
      FROM speed_data
      WHERE match = 1 AND samerace = 0
      ")
sqldf("SELECT COUNT(iid) 
      FROM tab05")
```