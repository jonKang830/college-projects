...
No black daters rate the importance of their datee being the same race as at least 8 out of 10.
...
```{r}
int_corr_success <- sqldf("
  SELECT *, ABS(int_corr) AS abs_int_corr
  FROM speed_data
  WHERE match=1
  ORDER BY abs_int_corr DESC
                            ")

sqldf("
  SELECT abs_int_corr, samerace, age, age_o, goal
  FROM int_corr_success
  LIMIT 10
      ")
```
