...
```{r}
library(tidyverse)
rentals_data <- read_csv("https://uofi.box.com/shared/static/2e2s3l35l2johejjvf0an39g9ucpxfw2.txt")
rentals_data <- rentals_data %>%
  mutate(`Inspection Date`=lubridate::mdy(`Inspection Date`),
         `Expiration Date`=lubridate::mdy(`Expiration Date`))
str(rentals_data)
```
...
```{r}
library(rvest)
property_url <- paste0("https://champaignil.devnetwedge.com/parcel/view/",rentals_data$`Parcel Number`,"/2020")
...
net_taxable[c(1,2,3,4,9)] #takes a long time if you attempt all 1730 parcels
```
...
```{r}
market_data <- read_delim("https://data.urbanaillinois.us/api/views/6gtk-bwms/rows.tsv?accessType=DOWNLOAD", "\t")
str(market_data)
```
...
market_data2 <- market_data %>%
  select(January,February,March,April,May,June,July,August,September,October,November,December, everything()) %>%
  arrange(Company, Product)
head(market_data2,10)
```
