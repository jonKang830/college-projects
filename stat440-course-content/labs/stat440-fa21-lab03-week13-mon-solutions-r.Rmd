...
```{r}
date_summary <- function(input){
  tibble(col1=input) %>%
    mutate(dates=col1, weekdays=lubridate::wday(col1, label=TRUE, abbr=FALSE), dayssince=floor(difftime(col1, '2011-01-01', units="days")), weekend=ifelse(weekdays=="Saturday"|weekdays=="Sunday",TRUE,FALSE)) %>%
    select(-col1)
}

sids <- sample(length(chicago_food_data$InspectionDate),20)
date_summary(chicago_food_data$InspectionDate[sids])
```
...
Based on my random sample, there are `r length(which(months(as.Date(chicago_food_data$InspectionDate)[sids])%in%c("June","July","August") ))` dates within months June, July, or August.

```{r}
length(which(months(as.Date(chicago_food_data$InspectionDate)[sids])%in%c("June","July","August") ))
```