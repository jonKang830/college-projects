...
chicago_food_data01 <- read_csv("https://uofi.box.com/shared/static/k6h51rdj07jmmiv11q3vmehgr2200eua.csv")
...
chicago_food_data <- sqldf("
      SELECT chicago_food_data01.`Inspection ID`, `DBA Name`, `AKA Name`, `License #`, `Facility Type`,  Risk, Address, City, State, Zip, `Inspection Date`, `Inspection Type`, Results, Violations, Latitude, Longitude
      FROM chicago_food_data01, chicago_food_data02
      WHERE chicago_food_data01.`Inspection ID` = chicago_food_data02.`Inspection ID`
      ")
...
```{r}
sqldf("SELECT COUNT(`Facility Type`) AS fac_freq, `Facility Type`
  FROM chicago_food_data
  WHERE Results='Fail'
  GROUP BY `Facility Type`
  ORDER BY fac_freq DESC
  LIMIT 10")
```